#!/bin/sh
# Experimental Rust installer; the Python reference installer is unchanged.
set -eu
...
BINARY=${AIUSAGE_BINARY:-"$SCRIPT_DIR/target/release/aiusage"}
...
owned_launcher() {
    [ -L "$1" ] && [ "$(readlink "$1")" = '../lib/aiusage/aiusage' ] && [ -f "$PACKAGE/.aiusage-rust-owned" ] && return 0
...
if { [ -e "$BINDIR/aiusage" ] || [ -L "$BINDIR/aiusage" ]; } && ! owned_launcher "$BINDIR/aiusage" 'aiusage.cli'; then
...
if [ -n "$existing_ai" ] && ! owned_launcher "$existing_ai" 'aiusage.manager'; then INSTALL_AI=0; fi
if { [ -e "$BINDIR/ai" ] || [ -L "$BINDIR/ai" ]; } && ! owned_launcher "$BINDIR/ai" 'aiusage.manager'; then INSTALL_AI=0; fi
...
cleanup() {
    ...
    if [ "$success" -eq 0 ] && [ "$publishing" -eq 1 ]; then
        ...
        if [ "$had_usage" -eq 1 ]; then mv -f "$stage/previous-usage" "$BINDIR/aiusage"; else rm -f "$BINDIR/aiusage"; fi
        if [ "$INSTALL_AI" -eq 1 ]; then
            if [ "$had_ai" -eq 1 ]; then mv -f "$stage/previous-ai" "$BINDIR/ai"; else rm -f "$BINDIR/ai"; fi
        fi
        if [ "$had_uninstaller" -eq 1 ]; then mv -f "$stage/previous-uninstaller" "$UNINSTALLER"; else rm -f "$UNINSTALLER"; fi
    fi
    rm -rf -- "$stage"
    exit "$result"
}
...
if [ -e "$BINDIR/aiusage" ] || [ -L "$BINDIR/aiusage" ]; then cp -pP "$BINDIR/aiusage" "$stage/previous-usage"; had_usage=1; fi
if [ "$INSTALL_AI" -eq 1 ] && { [ -e "$BINDIR/ai" ] || [ -L "$BINDIR/ai" ]; }; then cp -pP "$BINDIR/ai" "$stage/previous-ai"; had_ai=1; fi
if [ -e "$UNINSTALLER" ] || [ -L "$UNINSTALLER" ]; then cp -pP "$UNINSTALLER" "$stage/previous-uninstaller"; had_uninstaller=1; fi
publishing=1
if [ -d "$PACKAGE" ]; then mv "$PACKAGE" "$stage/previous-package"; old_package=1; fi
...
success=1
echo '✓ AIUsage 安装完成'
echo '管理菜单：aiusage --menu'
echo '直接启动额度看板：aiusage'
if [ "$INSTALL_AI" -eq 0 ]; then echo '第三方 ai 命令保持不变。'; fi
