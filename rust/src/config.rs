...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            auto_discover: true,
            real_providers: strings(&["codex", "grok"]),
            demo_providers: strings(DEMO_DEFAULT),
...
pub fn valid(values: &[String]) -> Vec<String> {
    let mut result = vec![];
    for value in values {
        if PROVIDERS.iter().any(|(key, _)| key == value) && !result.contains(value) {
...
pub fn parse(text: &str) -> Config {
    ...
    for (key, dest) in [
        ("language", &mut cfg.language),
        ("theme", &mut cfg.theme),
        ("position", &mut cfg.position),
        ("timezone", &mut cfg.timezone),
    ] {
        if let Some(value) = values.get(key) {
            *dest = value.trim_matches('"').into();
        }
    }
    cfg.auto_discover = values
        .get("auto_discover")
        .map(|v| !v.eq_ignore_ascii_case("false"))
        .unwrap_or(true);
    for (key, dest) in [
        ("real_providers", &mut cfg.real_providers),
        ("demo_providers", &mut cfg.demo_providers),
        ("disabled_providers", &mut cfg.disabled_providers),
    ] {
        if let Some(value) = values.get(key) {
    ...
    if !["en", "zh"].contains(&cfg.language.as_str()) {
        cfg.language = "zh".into();
    }
    if !THEMES.contains(&cfg.theme.as_str()) {
        cfg.theme = "white".into();
    }
    if !POSITIONS.contains(&cfg.position.as_str()) {
        cfg.position = "center".into();
    }
    if timezones::parse(&cfg.timezone).is_err() {
        cfg.timezone = "system".into();
    }
    if cfg.real_providers.is_empty() {
        cfg.real_providers = strings(&["codex", "grok"]);
    }
    if cfg.demo_providers.is_empty() {
        cfg.demo_providers = strings(DEMO_DEFAULT);
    }
    cfg.disabled_providers
        .retain(|k| !cfg.real_providers.contains(k));
    if values.contains_key("real_providers") && !values.contains_key("disabled_providers") {
        for key in ["codex", "grok"] {
            if !cfg.real_providers.iter().any(|s| s == key) {
                cfg.disabled_providers.push(key.into());
            }
        }
    }
    cfg
...
pub fn serialize(cfg: &Config) -> String {
    ...
    for (key, list) in [
        ("real_providers", &cfg.real_providers),
        ("demo_providers", &cfg.demo_providers),
        ("disabled_providers", &cfg.disabled_providers),
    ] {
        let items = valid(list)
            .iter()
            .map(|v| format!("\"{v}\""))
            .collect::<Vec<_>>()
            .join(", ");
...
mod tests {
    use super::*;
    #[test]
    fn legacy_and_explicit_disable() {
    ...
    #[test]
    fn permissive_not_standard_toml() {
        let cfg = parse(
            "language = en\nreal_providers=['grok', 'bogus', 'grok']\nauto_discover = FALSE\n",
        );
        assert_eq!(cfg.language, "en");
        assert_eq!(cfg.real_providers, vec!["grok"]);
