...
use crate::{
    config::Config,
    i18n::tr,
    models::{Availability, ProviderUsage},
    timezones, PROVIDERS,
};
use chrono::Datelike;
...
pub fn reset_text(epoch: Option<f64>, language: &str, timezone: &str) -> String {
    let Some(value) = epoch.and_then(|e| timezones::from_epoch(e, timezone).ok()) else {
        return "?".into();
    };
    let stamp = if language == "zh" {
        format!(
            "{}月{:02}日 {}",
            value.month(),
            value.day(),
...
fn provider_lines(
    ...
    let mut lines = vec![format!(
        ...
        if provider.stale {
            format!("  ! {}", tr(language, "stale"))
        } else {
            String::new()
        }
    )];
    if provider.availability != Availability::Available || provider.windows.is_empty() {
        let key = match provider.availability {
            Availability::Available => "available",
            Availability::NotInstalled => "not_installed",
            Availability::Unavailable => "unavailable",
            Availability::NotSupported => "not_supported",
        };
        ...
        for window in provider.windows.iter().take(2) {
            ...
            let label = if window.label == "Week" && language == "en" {
                "Weekly"
            } else {
                &window.label
            };
            ...
            lines.push(if visible_len(&full) <= width {
                full
            } else {
                value
            });
...
fn position(lines: Vec<String>, width: usize, height: usize, pos: &str) -> Vec<String> {
    ...
    let (vertical, horizontal) = if pos == "center" {
        ("center", "center")
    } else {
        pos.split_once('-').unwrap_or(("center", "center"))
    };
    let left = match horizontal {
        "left" => 0,
        "right" => width.saturating_sub(block_width),
        _ => width.saturating_sub(block_width) / 2,
    };
    let top = match vertical {
        "top" => 0,
        "bottom" => height.saturating_sub(lines.len()),
    ...
    result.extend(
        lines
            .iter()
            .map(|line| fit(&format!("{}{line}", " ".repeat(left)), width)),
    );
    result
}
...
pub fn dashboard(frame: Frame<'_>) -> Vec<String> {
    ...
    let title = if demo {
        format!("AI USAGE [{}]", tr(language, "demo"))
    } else {
        "AI USAGE".into()
    };
    if width < 4 || height < 3 {
        return position(vec![fit(&title, width)], width, height, &cfg.position);
    }
    let stamp = updated
        .and_then(|e| timezones::from_epoch(e, &cfg.timezone).ok())
    ...
    let cell = preferred.min(
        capacity
            .saturating_sub(2 * (columns - 1))
            .checked_div(columns)
            .unwrap()
            .max(1),
    );
    ...
    let outer = width.min(
        (grid + 4)
            .max(visible_len(tr(language, "help_compact")) + 4)
            .max(visible_len(&system) + 4),
    );
    let inner = outer - 2;
    let content_width = inner.saturating_sub(2).max(1);
    let cell = cell.min((content_width.saturating_sub(2 * (columns - 1)) / columns).max(1));
    let blocks: Vec<_> = providers
        .iter()
        .map(|p| provider_lines(p, cell, language, &cfg.timezone))
        .collect();
    ...
    for (row_index, group) in blocks.chunks(columns).enumerate() {
        ...
        for index in 0..row_height {
            let value = group
                .iter()
                .map(|b| pad(b.get(index).map(String::as_str).unwrap_or(""), cell))
                .collect::<Vec<_>>()
                .join("  ");
            content.push(format!(" {}{value}", " ".repeat(left)));
            kinds.push(if index == 0 { "strong" } else { "normal" });
        }
        if (row_index + 1) * columns < blocks.len() {
    ...
    let help = tr(
        language,
        if visible_len(tr(language, "help")) <= content_width.saturating_sub(1) {
            "help"
        } else {
            "help_compact"
        },
    ...
    if let Some(notice) = notice {
        content.push(format!(" {notice}"));
        kinds.push("strong");
    }
    content.extend([String::new(), format!(" {help}"), String::new()]);
    kinds.extend(["normal", "muted", "normal"]);
    content.truncate(height - 2);
    kinds.truncate(height - 2);
...
pub fn selector(
    ...
    cfg: &Config,
    color: bool,
    discovery: &std::collections::HashMap<String, String>,
) -> Vec<String> {
    ...
    for (index, (key, name)) in PROVIDERS.iter().enumerate() {
        ...
        let mut value = format!(
            ...
            order.map(|n| format!("  {n}")).unwrap_or_default()
        );
        if let Some(reason) = discovery.get(*key) {
            ...
            if available >= 5 {
                value.push_str(&format!(
                    "  {}",
                    fit(tr(&cfg.language, reason), available - 2)
                ));
            }
        }
...
fn selection_position(
    ...
    let count = lines.len();
    let mut lines = position(lines, width, height, "center");
    let top = lines.len() - count;
    style(&mut lines[top..], &vec!["normal"; count], &cfg.theme, color);
    lines
}
pub fn timezone_selector(
    ...
    let box_width = width.saturating_sub(4).clamp(34, 60);
    let inner = box_width - 2;
    let row = |s: &str| format!("│ {} │", pad(s, inner - 2));
    ...
    for (index, value) in options.iter().enumerate() {
        let label = if value == "system" {
            tr(&cfg.language, "system_zone").into()
        } else if index == options.len() - 1 {
            format!("{}: {value}", tr(&cfg.language, "custom_zone"))
        } else {
            value.clone()
        };
    ...
    lines.push(row(&fit(tr(&cfg.language, "timezone_help"), inner - 2)));
    lines.push(format!("└{}┘", "─".repeat(inner)));
    selection_position(lines, width, height, cfg, color)
}
