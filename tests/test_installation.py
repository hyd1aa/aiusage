        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with tarfile.open(fileobj=archive_bytes, mode="w:gz") as archive:
                for relative in ("install.sh", "uninstall.sh", "scripts", "src"):
                    archive.add(ROOT / relative, arcname=f"aiusage-{__version__}/{relative}")
            ...
            info = ReleaseInfo(__version__, "test", "", f"https://api.github.com/repos/hyd1aa/aiusage/tarball/v{__version__}")
            before = (metadata(bindir), metadata(libdir))
